...
```bash
make vendor-sparkle  # 下载 Sparkle xcframework 到 Vendor/（缺失时自动触发，首次构建前必需）
make build           # 编译 → build/aimeter.app（当前架构）
make universal       # 编译 arm64 + x86_64 通用二进制
make run             # 编译并启动
make install         # 安装到 ~/Applications/
make release         # universal + zip，用于发布
make notarize        # universal + Developer ID 签 + Apple 公证 + staple + zip
make clean           # 清理
swift test           # 跑单元测试（SPM，不走 Makefile，不链接 Sparkle）
```
...
5. GitHub Action (`.github/workflows/release.yml`) 自动：
   - `make vendor-sparkle` 拉取 Sparkle xcframework
   - 从 Secrets 恢复 Developer ID 证书到临时 keychain + notary API key 到文件
   - 编译 universal binary → inside-out Developer ID 签名（含 Sparkle 嵌套） → Apple 公证 → staple → 重新打 zip
   - 创建 GitHub Release
   - 用 Sparkle 官方 `generate_appcast` 基于现有 `main:docs/appcast.xml` 增量合并新版本，EdDSA 签名 → commit 回 main 分支 `docs/appcast.xml`
   - 跨 repo 推 Homebrew cask
   - 任一签名 secret 缺失时自动降级为 ad-hoc zip（**不更新 appcast**，保护现有用户不会升到未签名版本）
...
release.yml 使用 8 个 GitHub Secrets。签名相关的 7 个（前 7 行）缺任何一个都会自动降级为 ad-hoc 发布，同时跳过 appcast 更新：
...
| `SPARKLE_PRIVATE_KEY` | EdDSA 私钥文本（由 `generate_keys -x` 导出），Sparkle appcast/zip 签名用 |
...
**SPARKLE_PRIVATE_KEY**：`./Vendor/Sparkle/bin/generate_keys -x /tmp/sparkle_private.key`（本地只做一次，私钥已存 Keychain），然后 `gh secret set SPARKLE_PRIVATE_KEY --repo wangyufeng0615/aimeter < /tmp/sparkle_private.key && rm /tmp/sparkle_private.key`。**务必同时备份到密码管理器**——丢了就签不出 Sparkle 认的 appcast，老用户的自动更新全断，只能让他们重装。
...
- **公钥**：`Info.plist` 的 `SUPublicEDKey`（base64，绑死在 app 里，换了老用户就无法验签）。
- **私钥**：本地存 macOS Keychain（由 `generate_keys` 写入）；CI 用 GitHub Secret `SPARKLE_PRIVATE_KEY`。
- **appcast URL**：`SUFeedURL = https://raw.githubusercontent.com/wangyufeng0615/aimeter/main/docs/appcast.xml`（main 分支一个普通文件，不用 GitHub Pages）。
- **appcast 生成**：CI 用 Sparkle 官方 `generate_appcast` 工具，以 `origin/main:docs/appcast.xml` 为基底增量合并新版本条目，EdDSA 签名写回 main 分支。
- **版本比较**：Sparkle 用 `CFBundleVersion`（整数 build 号）判断新旧，不是 `CFBundleShortVersionString`。所以 build 号必须单调递增。
- **本地运行 ad-hoc build 需要 `com.apple.security.cs.disable-library-validation` entitlement**（`Resources/aimeter.entitlements`）：Sparkle 原厂签名和本地 ad-hoc 签名的 Team ID 不一致，Hardened Runtime 的 Library Validation 会拒绝加载。Developer ID 发布包在 inside-out codesign 时把 Sparkle 重签成同一个 Team ID，不需要这个豁免，Makefile 对两种模式用两套 `CODESIGN_APP` flags 自动处理。
- **vendored xcframework**：`Vendor/Sparkle/`（gitignored），`make vendor-sparkle` 会从 GitHub Release 下载。本地 clone 后如果在国内可能慢，可以 `curl -x <proxy> ...` 手工下载解压。
